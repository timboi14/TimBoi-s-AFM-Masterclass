...
/**
 * ACCA AFM Capability Map — every A1..E5 capability with its mapped TBA topic
 * and drill. Greyed rows are coverage gaps (next content sprint targets).
 *
 * Work Item 11 of the Platinum-tier upgrade.
 */
...
export function SyllabusPage() {
  ...
  const totalCovered = AFM_SYLLABUS.filter((r) => r.tbaTopicId).length;

  return (
    <>
      <SectionShell tone="white" pad="lg">
        <CenteredHero
          eyebrow={<>{AFM_SYLLABUS.length} capabilities · {totalCovered} covered</>}
          headline={<>The <HeroGold>capability map</HeroGold>.</>}
          subline={
            <>
              Every ACCA AFM Study Guide capability (A1 → E5), mapped to its TimBoi topic and
              drill. Greyed rows are the next content-sprint targets — no coverage yet.
            </>
          }
        />
      </SectionShell>
      ...
      <SectionShell tone="mist" pad="lg">
        {sections.map((sec) => (
          <div key={sec} className="mb-8">
            <h2 className="font-display text-xl tracking-wide uppercase text-ink mb-3">
              Section {sec}
            </h2>
